...
public class DairyAdapter extends RecyclerView.Adapter<DairyAdapter.ViewHolder> {
    ...
    public DairyAdapter(Context cleaner_context, List<DairyProduct> data) {
    ...
    public void onBindViewHolder(@NonNull final ViewHolder holder, final int position) {
        ...
        SharedPreferences sharedFavorit = cleaner_context.getSharedPreferences("dairylol", Context.MODE_PRIVATE);
        Boolean a = sharedFavorit.getBoolean("dairyabc" + position, false);
        ...
        holder.cleanerFavorite.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if (isChecked) {
                    holder.cleanerFavorite.setBackgroundDrawable(ContextCompat.getDrawable(cleaner_context, R.drawable.toogel_favourit_true));
                    SharedPreferences.Editor editor = cleaner_context.getSharedPreferences("dairylol", Context.MODE_PRIVATE).edit();
                    editor.putBoolean("dairyabc" + position, true);
                    editor.commit();
...
//                    Toast.makeText(cleaner_context, "done", Toast.LENGTH_SHORT).show();
                    ...
                    holder.cleanerFavorite.setBackgroundDrawable(ContextCompat.getDrawable(cleaner_context, R.drawable.toogel_ic_favourite));
                    SharedPreferences.Editor editor = cleaner_context.getSharedPreferences("dairylol", Context.MODE_PRIVATE).edit();
                    editor.putBoolean("dairyabc" + position, false);
                    editor.commit();
